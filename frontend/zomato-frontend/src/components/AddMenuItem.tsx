...
const AddMenuItem = ({onItemAdded}:{onItemAdded:() => void}) => {
  ...
  return (
    <div  className='max-w-md space-y-4 m-auto   '>
        <h2 className='tetx-lg font-semibold'>Add Menu Item</h2>
        ...
        <button disabled={loading} onClick={handleSubmit} className='w-full rounded-lg 
        text-white text-sm py-3 font-semibold transition bg-red-500 cursor-pointer'>{loading ? "Adding..." : "Add Item"}</button>
    </div>
  )
}
